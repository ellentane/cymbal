...
pub const PARAM_NAMES: &[&str] = &[
    "pan", "vel", "delay", "reverb", "bass", "treble", "comp", "swing", "start", "end", "dur",
    "cycle",
];
...
const PARAMS: &[Entry] = &[
    Entry {
        name: "pan",
        description: "stereo position, -1 (left) to 1 (right); also accepts a ramp a..b across the bar",
        example: "pan=-0.4",
    },
    Entry {
        name: "vel",
        description: "scales hit velocity, 0..=1; multiplies the per-hit '*' velocity",
        example: "vel=0.9",
    },
    Entry {
        name: "delay",
        description: "delay send, 0..=1; accepts a ramp",
        example: "delay=0.25",
    },
    Entry {
        name: "reverb",
        description: "reverb send, 0..=1; accepts a ramp",
        example: "reverb=0.4",
    },
    Entry {
        name: "bass",
        description: "per-voice low shelf, 0..=1",
        example: "bass=0.5",
    },
    Entry {
        name: "treble",
        description: "per-voice high shelf, 0..=1",
        example: "treble=0.5",
    },
    Entry {
        name: "comp",
        description: "per-voice compressor, 0..=1",
        example: "comp=0.3",
    },
    Entry {
        name: "swing",
        description: "delays the odd 8th steps, 0..=0.5",
        example: "swing=0.25",
    },
    Entry {
        name: "start",
        description: "sample region start (0..=1), sample voices only",
        example: "start=0.25",
    },
    Entry {
        name: "end",
        description: "sample region end (0..=1), sample voices only",
        example: "end=0.75",
    },
    Entry {
        name: "dur",
        description: "trigger length in seconds (0..=60), sample voices only",
        example: "dur=0.2",
    },
    Entry {
        name: "cycle",
        description: "loop the sample region for the trigger duration (0 or 1), sample voices only",
        example: "cycle=1",
    },
];
...
const SYMBOLS: &[Entry] = &[
    Entry {
        name: "<<",
        description: "send a pattern to a voice",
        example: "kick << \"x . x .\"",
    },
    Entry {
        name: "|",
        description: "chain a transform onto a pattern; combine freely",
        example: "\"x . x .\" | rev",
    },
    Entry {
        name: "*",
        description: "per-hit velocity scale, 0..=1",
        example: "x*0.5",
    },
    Entry {
        name: "+",
        description: "transpose a hit up N semitones",
        example: "x+2",
    },
    Entry {
        name: "-",
        description: "transpose a hit down N semitones",
        example: "x-2",
    },
    Entry {
        name: "..",
        description: "a ramp from a to b across each bar, in a param",
        example: "pan=-0.5..0.5",
    },
    Entry {
        name: "[ ]",
        description: "a note array — evenly spaced triggers, one pitch each",
        example: "[c4, d4, e4, g4]",
    },
    Entry {
        name: " ",
        description: "a rhythm pattern — x hits, . rests, length is the subdivision",
        example: "\"x . . x\"",
    },
    Entry {
        name: "=",
    ...
    Entry {
        name: ":",
        description: "loop header terminator",
        example: "loop \"beat\":",
    },
    Entry {
        name: ",",
        description: "separates list items and every(n, rev) arguments",
        example: "[c4, d4] every(4, rev)",
    },
    Entry {
        name: "--",
    ...
    Entry {
        name: "x",
        description: "a hit inside a rhythm pattern",
        example: "x",
    },
    Entry {
        name: ".",
        description: "a rest inside a rhythm pattern",
        example: ".",
    },
];
...
const KEYWORDS: &[Entry] = &[
    Entry {
        name: "let",
        description: "define a voice by name; names bind (declared wins over built-ins)",
        example: "let kick = kick()",
    },
    Entry {
        name: "loop",
        description: "open a repeating loop block; binds live inside it",
        example: "loop \"beat\":",
    },
    Entry {
        name: "tempo",
        description: "set the transport tempo, 20..=4000",
        example: "tempo 120",
    },
    Entry {
        name: "sample",
        description: "load a sample voice from a WAV file or the bundled kit",
        example: "let clap = sample \"clap\"",
    },
    Entry {
        name: "every",
        description: "apply a transform on every nth cycle",
        example: "| every(4, rev)",
    },
    Entry {
        name: "rev",
        description: "reverse a pattern's steps",
        example: "| rev",
    },
    Entry {
        name: "help",
        description: "show help for a topic, or the whole cheat sheet",
        example: "help pan",
    },
];
...
const VOICES: &[Entry] = &[
    Entry {
        name: "kick",
        description: "kick drum voice",
        example: "kick()",
    },
    Entry {
        name: "snare",
        description: "snare voice",
        example: "snare()",
    },
    Entry {
        name: "hat",
        description: "hi-hat voice",
        example: "hat()",
    },
    Entry {
        name: "bass",
    ...
    Entry {
        name: "sample",
        description: "sample voice from a WAV file or the bundled kit",
        example: "sample \"kick\"",
    },
];
        ...
        if let Stmt::Help(topic, _) = stmt {
            return Some(match topic {
                Some(t) => {
                    ...
                    match lookup(t) {
                        Some(e) => {
                            format!("{} — {}\n  example: {}", e.name, e.description, e.example)
                        }
...
mod tests {
    ...
    fn name_constants_match_tables() {
        ...
        for p in PARAM_NAMES {
            assert!(
                param_entries.contains(p),
                "PARAM_NAMES '{p}' missing from PARAMS table"
            );
        }
        for v in VOICE_NAMES {
            assert!(
                voice_entries.contains(v),
                "VOICE_NAMES '{v}' missing from VOICES table"
            );
        }
    }
}
